...
def iperf(gbps):
    return {
        "end": {
            "sum_received": {"bits_per_second": gbps * 1e9},
            "sum_sent": {"bits_per_second": gbps * 1e9, "retransmits": 2},
        }
    }
...
class ModeBAnalyzerTests(unittest.TestCase):
    def build_experiment(self, root):
        ...
        for run_id in range(1, 4):
            ...
            for condition, shift in (("peering", 0.0), ("tgw", 0.7)):
                ...
                (path.parent / "ping.txt").write_text(
                    "10 packets transmitted, 10 received, 0% packet loss\n"
                    "rtt min/avg/max/mdev = 0.100/0.200/0.300/0.020 ms\n",
                    encoding="utf-8",
                )
            ...
            events = []
            for load, saturated in ((100000, False), (250000, True)):
                for condition, achieved in (("iptables", load * 0.90), ("xdp", load * 0.98)):
                    events.append({
                        "run_id": run_id,
                        "load_target_pps": load,
                        "condition": condition,
                        "saturation": {
                            "saturated": saturated,
                            "achieved_load_pps": achieved + run_id,
                            "dut_ena_rx_pps": achieved - 100 + run_id,
                            "loss_pct": 2.0 if saturated else 0.0,
                            "p99_ms": 6.0 if saturated else 0.5,
                            "cpu_pct": 91.0 if saturated else 50.0,
                            "softirq_pct": 40.0,
                            "violations": ["legitimate_loss_pct"] if saturated else [],
                        },
                    })
            saturation = run / "tc03" / "saturation_events.jsonl"
            saturation.parent.mkdir(parents=True, exist_ok=True)
            saturation.write_text("\n".join(json.dumps(event) for event in events) + "\n", encoding="utf-8")
            commands = run / "commands"
            commands.mkdir()
            for index in (1, 2):
                (commands / f"xdp_{index}_stdout.log").write_text(
                    "___XDP_NATIVE_EVIDENCE_BEGIN___\n"
                    "driver: ena\nprog/xdp id 42\n42: xdp name test tag abc type xdp\n"
                    "___XDP_NATIVE_EVIDENCE_END___\n",
                    encoding="utf-8",
                )
            seal(run)
    ...
    def test_unified_summary_covers_all_testcases_and_uses_honest_method_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([key for key in ("tc01", "tc02", "tc03", "tc04") if key in result], ["tc01", "tc02", "tc03"])
            self.assertFalse(result["tc01"]["design"]["same_backend_all_runs"])
            self.assertEqual(len(result["tc02"]["cells"]), 6)
            self.assertIsNotNone(result["tc03"]["paired_max_sustainable_pps_xdp_minus_iptables"])
    ...
    def test_tc03_generic_xdp_evidence_fails_protocol_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "experiment_fixture"
            root.mkdir()
            self.build_experiment(root)
            run = root / "run_001"
            evidence = run / "commands" / "xdp_1_stdout.log"
            evidence.write_text(evidence.read_text(encoding="utf-8") + "xdpgeneric\n", encoding="utf-8")
            seal(run)
            with self.assertRaises(mode_b.DataIntegrityError):
                mode_b.analyze_experiment(root, n_resamples=20)
